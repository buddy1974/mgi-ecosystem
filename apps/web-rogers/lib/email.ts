...
// ─── AI assistant lead → admin ──────────────────────────────────────────────

export function sendAssistantLead(params: {
  name: string
  email: string
  interest: string
}): void {
  const { name, email, interest } = params

  void sendEmail({
    to: ADMIN_TO,
    replyTo: email,
    subject: `🤖 AI Lead — ${name}${interest ? ` — ${interest}` : ''}`,
    html: `
<!DOCTYPE html><html><body style="margin:0;padding:0;background:#f4f4f8;font-family:'Segoe UI',system-ui,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:40px 16px;">
<table width="540" cellpadding="0" cellspacing="0" style="max-width:540px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e0e0ef;">
  <tr><td style="background:linear-gradient(135deg,#f72585,#7209b7);padding:28px 32px;">
    <p style="color:rgba(255,255,255,0.7);font-size:11px;letter-spacing:0.3em;text-transform:uppercase;margin:0 0 8px;">Rogers AI — Lead Captured</p>
    <h1 style="color:#ffffff;font-size:20px;font-weight:700;margin:0;">${name} wants priority updates</h1>
  </td></tr>
  <tr><td style="padding:28px 32px;">
    <table width="100%" cellpadding="0" cellspacing="0">
      ...
      <tr><td style="padding:10px 0;border-bottom:1px solid #f0f0f5;color:#666;font-size:13px;">Email</td><td style="padding:10px 0;border-bottom:1px solid #f0f0f5;color:#f72585;font-size:13px;"><a href="mailto:${email}" style="color:#f72585;">${email}</a></td></tr>
      <tr><td style="padding:10px 0;color:#666;font-size:13px;">Interest</td><td style="padding:10px 0;color:#1a1a2e;font-size:13px;font-weight:600;">${interest || 'Not specified'}</td></tr>
    </table>
    <p style="color:#999;font-size:12px;margin:24px 0 0;">Reply directly to this email to follow up with ${name}. They came through the AI assistant.</p>
  </td></tr>
  <tr><td style="padding:20px 32px;background:#f8f8fd;border-top:1px solid #e0e0ef;">
    <p style="color:#999;font-size:12px;margin:0;text-align:center;">Rogers Nforgwei · AI Assistant Lead · rogersnforgwei.com</p>
  </td></tr>
</table>
</td></tr></table>
</body></html>
